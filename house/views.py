...
from house.models import Property,Profile,Tenant
...
def owner(request):
	...
	return render(request, "owner_site.html",{'myproperty':myproperty,'name':name,'allp':allp})

def allp(request):
	return render(request,"allp.html")
...
def tenant(request):
	...
	print(myproperty)
	gg=Property.objects.all()

	return render(request, "tenant_site.html",{'name':name,'myproperty':myproperty,'gg':gg})
...
def add_property(request):
	if request.method == "POST":
		...
		print(image)
		user = request.user
		title = Property(name=name,address=address,description=description,amount=amount,image=image,owner=user)
		title.save()
